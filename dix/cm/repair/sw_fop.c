...
static int dix_repair_sw_fom_create(struct m0_fop *fop, struct m0_fom **m,
				    struct m0_reqh *reqh)
{
	struct m0_fop *r_fop;
	int            rc;

	r_fop = m0_fop_reply_alloc(fop, &dix_repair_sw_onwire_rep_fopt);
	if (r_fop == NULL)
...
const struct m0_fom_type_ops dix_repair_sw_fom_type_ops = {
	.fto_create = dix_repair_sw_fom_create
};

/** @see m0_cm_repreb_sw_onwire_fop_init() */
M0_INTERNAL void m0_dix_repair_sw_onwire_fop_init(void)
{
        m0_cm_repreb_sw_onwire_fop_init(&dix_repair_sw_onwire_fopt,
					...
					&dix_repair_cmt);
        m0_cm_repreb_sw_onwire_fop_init(&dix_repair_sw_onwire_rep_fopt,
					&dix_repair_sw_fom_type_ops,
...
{
	m0_cm_repreb_sw_onwire_fop_fini(&dix_repair_sw_onwire_fopt);
	m0_cm_repreb_sw_onwire_fop_fini(&dix_repair_sw_onwire_rep_fopt);
}
...
{
	return m0_cm_repreb_sw_onwire_fop_setup(cm, &dix_repair_sw_onwire_fopt,
