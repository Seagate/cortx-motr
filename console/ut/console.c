...
static int console_cmd(char **params)
{
	...
	for (numparams = 0; params[numparams] != NULL; ++numparams)
		;

	return console_main(numparams, params);
}
...
static void console_input_test(void)
{
	...
	result = console_cmd(ALIST("no_input"));
	...
	result = console_cmd(ALIST("no_input", "-v"));
	M0_UT_ASSERT(result == EX_USAGE);
	M0_UT_ASSERT(m0_error_mesg_match(stderr, usage_msg));
	result = truncate(err_file, 0L);
	M0_UT_ASSERT(result == 0);
	...
	result = console_cmd(ALIST("list_fops", "-l"));
	M0_UT_ASSERT(result == EX_USAGE);
	...
	result = console_cmd(ALIST("show_fops", "-l", "-f", buf));
	...
	result = console_cmd(ALIST("show_fops", "-l", "-f", 0));
	M0_UT_ASSERT(result == EX_USAGE);
	M0_UT_ASSERT(m0_error_mesg_match(stderr, usage_msg));
	result = truncate(err_file, 0L);
	M0_UT_ASSERT(result == 0);
	...
	result = console_cmd(ALIST("yaml_input", "-i"));
	M0_UT_ASSERT(result == EX_USAGE);
	M0_UT_ASSERT(m0_error_mesg_match(stderr, usage_msg));
	result = truncate(err_file, 0L);
	M0_UT_ASSERT(result == 0);
	...
	result = console_cmd(ALIST("yaml_input", "-y", (char *)yaml_file));
	M0_UT_ASSERT(result == EX_USAGE);
	M0_UT_ASSERT(m0_error_mesg_match(stderr, usage_msg));
	result = truncate(err_file, 0L);
	M0_UT_ASSERT(result == 0);
	...
	result = console_cmd(ALIST("yaml_input", "-i", "-y", (char *)yaml_file));
