 ...
 *
 * Motr index API defines a Key-Value interface supporting only a few simple
 ...
 * (1) Motr index interface requires K-V pairs are sorted in order by keys,
 *     particularly for NEXT operation.
 *
 * (2) A Motr application may use the index API in different ways, taking S3
 *     server, an application which uses Motr index API and Cassandra as a
 ...
 * (4) A mapping layer is introduced to translate Motr index to a physical
 ...
 *     example, Motr can map S3 bucket indices(object list) into one or more
