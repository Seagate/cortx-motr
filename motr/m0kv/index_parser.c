...
static int vals_load(const char *value, struct m0_bufvec *vals)
{
	...
	if (value[0] == '@') {
		...
		keylen = strlen(value);
		vals->ov_buf[0] = m0_alloc(keylen);
		if (vals->ov_buf[0] == NULL)
			return M0_ERR(-errno);

		if (is_str) {
			memcpy(vals->ov_buf[0], value, keylen);
			vals->ov_vec.v_count[0] = keylen;
			vals->ov_vec.v_nr = 1;
		} else {
