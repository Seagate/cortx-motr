...
static void ut_test_data_buf_init(void)
{
	...
	addr = (void *)(DUMMY_PTR & (~M0_NETBUF_MASK));
	M0_ALLOC_PTR(buf);
...
static void ut_pargrp_iomap_free_data_buf(struct pargrp_iomap *map,
					  int row, int col)
{
	ut_dummy_data_buf_fini(map->pi_databufs[row][col]);
...
static void ut_test_pargrp_iomap_populate(void)
{
	...
	ut_pargrp_iomap_free_data_buf(map, 0, 0);
	...
	ut_pargrp_iomap_free_data_buf(map, 0, 0);
	ut_pargrp_iomap_free_data_buf(map, 0, 1);
	...
	ut_pargrp_iomap_free_data_buf(map, 0, 0);
	ut_dummy_paritybufs_create(map, false);
...
static void ut_test_pargrp_iomap_fini(void)
{
	...
	pargrp_iomap_bob_init(map);
	map->pi_ops = (struct pargrp_iomap_ops *)DUMMY_PTR;
	map->pi_ioo = (struct m0_op_io *)DUMMY_PTR;
...
/* Workaround to reproduce some functionality from layout/layout.c */
...
static void ut_test_pargrp_iomap_pages_mark_as_failed(void)
{
	...
	M0_UT_ASSERT(rc == 0);
	ut_dummy_paritybufs_delete(map, false);
...
static void ut_test_pargrp_iomap_dgmode_recover(void)
{
	...
	math->pmi_data_count = M0T1FS_LAYOUT_N;
	math->pmi_parity_count = M0T1FS_LAYOUT_K;
	...
	for (i = 0; i < M0T1FS_LAYOUT_N; i++)
		map->pi_databufs[0][i]->db_flags = 0;
	for (i = 0; i < M0T1FS_LAYOUT_K; i++)
...
#ifndef __KERNEL__
	ut_shuffle_test_order(&ut_suite_io_pargrp);
#endif
...
struct m0_ut_suite ut_suite_io_pargrp = {
	.ts_name = "io-pargrp-ut",
	.ts_init = ut_io_pargrp_init,
	.ts_fini = ut_io_pargrp_fini,
	.ts_tests = {
		{ "data_buf_invariant",
