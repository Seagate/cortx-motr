			       ...
			       struct m0_client *instance)
{
	...
	ut_striped_layout_fini(&pl->pl_base,
			       &instance->m0c_reqh.rh_ldom);
	m0_free(pl);
}
					    ...
					    bool do_alloc)
{
	...
	for (i = 0; i < map->pi_max_row; i++) {
		M0_ALLOC_ARR(map->pi_paritybufs[i], M0T1FS_LAYOUT_K);
					    ...
					    bool do_free)
{
	...
	for (i = 0; i < map->pi_max_row; i++) {
		for (j = 0; j < M0T1FS_LAYOUT_K && do_free == true; j++) {
			ut_dummy_data_buf_fini(map->pi_paritybufs[i][j]);
...
ut_dummy_pargrp_iomap_create(struct m0_client *instance, int num_blocks)
{
	...
	M0_UT_ASSERT(num_blocks <= M0T1FS_LAYOUT_N);
	...
	M0_ALLOC_ARR(ret->pi_databufs, M0T1FS_LAYOUT_N);
	ret->pi_max_row = 1;
	ret->pi_max_col = M0T1FS_LAYOUT_N;

	for (r = 0; r < ret->pi_max_row; r++) {
		M0_ALLOC_ARR(ret->pi_databufs[r], M0T1FS_LAYOUT_N);
		...
		for (c = 0; c < ret->pi_max_col; c++) {
			...
			ut_dummy_data_buf_init(ret->pi_databufs[r][c]);
		}
	}

	ret->pi_ioo = (struct m0_op_io *)DUMMY_PTR;
			     ...
			     struct m0_client *instance)
{
	...
	for (r = 0; r < map->pi_max_row; r++) {
		for (c = 0; c < map->pi_max_col; c++) {
			ut_dummy_data_buf_fini(map->pi_databufs[r][c]);
...
ut_dummy_ioo_create(struct m0_client *instance, int num_io_maps)
{
	...
	m0_op_obj_bob_init(&ioo->ioo_oo);
	m0_op_common_bob_init(&ioo->ioo_oo.oo_oc);
	m0_op_bob_init(&ioo->ioo_oo.oo_oc.oc_op);
	...
	pdi = ut_dummy_pdclust_instance_create(pl);
	...
	ut_dummy_xfer_req_init(&ioo->ioo_nwxfer);
	...
	for (i = 0; i < ioo->ioo_iomap_nr; i++) {
		ioo->ioo_iomaps[i] =
			ut_dummy_pargrp_iomap_create(instance,
						     M0T1FS_LAYOUT_N);
				     ...
				     struct m0_client *instance)
{
	...
	ut_dummy_xfer_req_fini(&ioo->ioo_nwxfer);
...
M0_INTERNAL struct target_ioreq *ut_dummy_target_ioreq_create(void)
{
	...
	ti->ti_coff_ivec.iv_index = (void *)DUMMY_PTR;
