...
static bool node__invariant(const struct nd *node)
{
...
static void node_rec(struct slot *slot)
{
	M0_PRE(node__invariant(slot->s_node));
...
static void node_key(struct slot *slot)
{
	M0_PRE(node__invariant(slot->node));
...
#ifndef __KERNEL__
M0_UNUSED static void node_find(struct slot *slot, const struct m0_btree_key *key)
{
	M0_PRE(node__invariant(slot->s_node));
...
static void node_move(struct nd *src, struct nd *tgt,
		      enum dir dir, int nr, struct m0_be_tx *tx)
{
	M0_PRE(node__invariant(src));
	M0_PRE(node__invariant(dst));
 ...
 * @return Next state to proceed in.
 */
M0_UNUSED static int64_t tree_create(struct node_op *op, struct m0_btree_type *tt,
			   int rootshift, struct m0_be_tx *tx, int nxt)
{
 ...
 * @return Next state to proceed in.
 */
M0_UNUSED static int64_t tree_delete(struct node_op *op, struct td *tree,
			   struct m0_be_tx *tx, int nxt)
{
	M0_PRE(tree != NULL);
...
#ifndef __KERNEL__
M0_UNUSED static int64_t node_free(struct node_op *op, struct nd *node,
			 struct m0_be_tx *tx, int nxt)
{
