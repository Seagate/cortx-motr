...
#define M0_BTREE_TX_CAPTURE(tx, seg, ptr, size)                              \
	do { (tx) = (tx); (seg) = (seg); (ptr) = (ptr); } while (0)
...
struct node_type {
	...
	bool (*nt_capture)(struct slot *slot, struct m0_be_tx *tx);
...
static void node_move (struct nd *src, struct nd *tgt, enum dir dir, int nr,
		       struct m0_be_tx *tx);
#endif
static bool node_capture(struct slot *slot, struct m0_be_tx *tx);
/**
 * Common node header.
 *
...
static bool node_capture(struct slot *slot, struct m0_be_tx *tx)
{
	return slot->s_node->n_type->nt_capture(slot, tx);
}
			       ...
			       int rootshift, struct m0_be_tx *tx, int nxt)
{
	...
	tree = op->no_tree;
	node_alloc(op, tree, rootshift, &fixed_format, 8, 8, false, tx, nxt);
...
static bool ff_capture(struct slot *slot, struct m0_be_tx *tx);
	...
	if (oi->i_used == 0) {
		...
		if (oi->i_nop.no_node == NULL) {
			...
			return node_alloc(&oi->i_nop, tree, shift,
					  lev->l_node->n_type, ksize, vsize,
					  lock_acquired, bop->bo_tx, P_ALLOC);
		}
		if (oi->i_nop.no_op.o_sm.sm_rc == 0) {
			lev->l_alloc = oi->i_nop.no_node;
			oi->i_nop.no_node = NULL;
...
static void ut_node_create_delete(void)
{
	...
	node_alloc(&op1, tree, 10, nt, 8, 8, false, NULL, 0);
	node1 = op1.no_node;

	op2.no_opc = NOP_ALLOC;
	node_alloc(&op2,  tree, 10, nt, 8, 8, false, NULL, 0);
	node2 = op2.no_node;
...
static void ut_multi_stream_kv_oper(void)
{
	...
	cred = M0_BE_TX_CREDIT(50, 200 * 1024);
