...
SCRIPT_START=0
HOME_DIR=""
MOTR_REPO="http://gerrit.motr.colo.seagate.com/motr"
HARE_REPO="http://gitlab.motr.colo.seagate.com/motr/hare.git"
GIT_LAB="http://gitlab.motr.colo.seagate.com"
CL_DIR_TAG=""
MOTR_REF=""
HARE_USER=""
HARE_USER_BRANCH=""
...
VMHOST_PATH_IN_VM="/data/motr/scripts/provisioning/vmhost"
COMPILE_INSTALL_MOTR_SCRIPT="$VMHOST_PATH_IN_VM/gci-compile-install-motr.sh"
...
print_usage()
{
    ...
    echo -e "Using MOTR_REPO [$MOTR_REPO]"
    echo -e "Using M0_VM_BOX_URL [$M0_VM_BOX_URL]"
    echo -e "\n"
    echo -e "./gci-create-cluster.sh <CLUSTER_NAME> <MOTR_REF> <HARE_USER> "  \
        "<HARE_USER_BRANCH>"
    echo -e "\t On dev HEAD                                        "       \
        "./gci-create-cluster.sh CL0001 \"\" \"\" \"\""
    echo -e "\t On dev with MOTR_REF                               "       \
        "./gci-create-cluster.sh CL0001 \"refs/changes/28/17128/1\" \"\""
    echo -e "\t On dev with HARE_USER and HARE_USER_BRANCH         "       \
        "./gci-create-cluster.sh CL0001 \"\" \"sourish.banerje\" "            \
        "\"alpha_pct\""
    echo -e "\t On dev with MOTR_REF & HARE_USER, HARE_USER_BRANCH "       \
        "./gci-create-cluster.sh CL0001 \"refs/changes/28/17128/1\" "         \
...
clone_motr()
{
    ...
    print_msg "Executing [git clone --recursive $MOTR_REPO]"
    git clone --recursive $MOTR_REPO
    RESULT=$?
    if [ $RESULT != 0 ]; then
        print_msg "ERROR checking out $MOTR_REPO!!!"
        print_status_and_time  $FSTART $RESULT
        return $RESULT
    fi

    # print_msg "EXECUTING FROM SOURISH's PRIVATE BRANCH !!!"; sleep 5
    # print_msg "Executing [cd motr]"
    # print_msg "Executing [git checkout dev/SB/PREP-SCRIPT-HARE; cd -;]"
    # cd motr
    # git checkout dev/SB/PREP-SCRIPT-HARE; cd -;

    if [ "$MOTR_REF" == "" ]; then
        print_msg "MOTR_REF was not specified. Compiling  HEAD of Motr " \
            "dev branch."
        print_status_and_time  $FSTART $RESULT
        return $RESULT
    fi
...
clone_hare()
{
    ...
    print_msg "Executing [git checkout origin/dev]"
...
create_vms()
{
    ...
    print_msg " Press 4 in 30 seconds to create all four VMs. " \
        "Else only cmu will be created."
    read -t 30 CH
    if [ "$CH" != "4" ]; then
...
create_cluster()
{
    ...
    print_msg "Using MOTR_REPO [$MOTR_REPO]; MOTR_REF [$MOTR_REF];"
    ...
    RESULT=$?
    if [ $RESULT != 0 ]; then
        print_msg "Function clone_motr(...) failed !!!"
        print_status_and_time  $FSTART $RESULT
        return $RESULT
    fi
...
print_msg "Now your Motr & Hare cluster will be created, git cloned, patched/"
...
RESULT=$?
if [ $RESULT != 0 ]; then
    print_msg "compile_install_motr(...) failed. Cannot continue further."
    script_terminate $RESULT
fi
