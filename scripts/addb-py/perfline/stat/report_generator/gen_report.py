...
def parse_time(stat_result_dir):
    ...
    if isfile(join(stat_result_dir, 'common_stats.yaml')):
        with open(join(stat_result_dir, 'common_stats.yaml'), 'r') as common_stats:
            ...
            workload_start_time = datetime.datetime.fromtimestamp(common_stats['workload_start_time']).strftime("%A, %d %B %Y %I:%M:%S%p")
            workload_stop_time = datetime.datetime.fromtimestamp(common_stats['workload_stop_time']).strftime("%A, %d %B %Y %I:%M:%S%p")
            test_start_time = datetime.datetime.fromtimestamp(common_stats['test_start_time']).strftime("%A, %d %B %Y %I:%M:%S%p")
            test_stop_time = datetime.datetime.fromtimestamp(common_stats['test_stop_time']).strftime("%A, %d %B %Y %I:%M:%S%p")
        ...
        if isdir(join(path, 'hw')):
            ...
            if isfile(join(path, 'hw/git_info')):
                with open(join(path, 'hw/git_info'), 'r') as commit_info_file:
                    ...
                    lines = [parse_commit_info(line) for line in lines if 'cortx' in line]
...
def parse_config_info(report_dir):
    ...
    if isfile(join(report_dir, 'm0d/configs/motr')):
        with open(join(report_dir, 'm0d/configs/motr'), 'r') as motr_config_file:
            ...
            motr_config = [conf for conf in motr_config if len(conf.split('=')) == 2 and conf[0] != '#']
...
def parse_s3_info(report_dir):
    ...
    s3_haproxy_cfg_files = [join(join(s3_dir, d), 'haproxy/cfg/haproxy.cfg') for d in listdir(s3_dir) if isdir(join(s3_dir, d))]
    ...
    s3_hosts_files = [join(join(s3_dir, d), 'hosts') for d in listdir(s3_dir) if isdir(join(s3_dir, d))]
    ...
    s3_config_files = [join(join(s3_dir, d), 's3config.yaml') for d in listdir(s3_dir) if isdir(join(s3_dir, d))]
    for path in s3_config_files:
        if isfile(path):
            with open(path, 'r') as s3_config_file:
                lines = s3_config_file.readlines()
                lines = [line.split('#')[0] if '#' in line else line for line in lines if line[0] != '#']
...
def parse_report_info(report_dir):
    ...
    if isdir(S3BENCH_TEST_DIR):
        path_to_rw_stats = [(f, join(S3BENCH_TEST_DIR, f)) for f in listdir(S3BENCH_TEST_DIR) if isfile(join(S3BENCH_TEST_DIR, f))]
        ...
        for fname, path in path_to_rw_stats:
            ...
            test_parameters = next(unfiltered_report[i+1:i+1+test_parameters_length] for i, l in enumerate(unfiltered_report) if l == 'Test parameters\n')
            write_summary = next(unfiltered_report[i+1:i+1+report_length] for i, l in enumerate(unfiltered_report) if l == 'Results Summary for Write Operation(s)\n')
            read_summary = next(unfiltered_report[i+1:i+1+report_length] for i, l in enumerate(unfiltered_report) if l == 'Results Summary for Read Operation(s)\n')
    ...
    workload_filenames = [f for f in listdir(workload_dir) if isfile(join(workload_dir, f))]
...
def parse_dstat_info(nodes_stat_dirs):
    ...
    for path in nodes_stat_dirs:
        if isfile(f'{path}/dstat/dstat.csv'):
            df = pd.read_csv(f'{path}/dstat/dstat.csv', sep=',', skiprows = lambda x: x in range(5))
            ...
            if ':' in net_columns[0]:
                ...
                for column in net_columns:
                    ...
                    plt.cla()
                dstat_net_info.append([column.replace(":", "_") for column in net_columns])
            else:
                # dstat 0.7.2
                net_columns_7_2 = []
                for column in net_columns:
...
def main():
    ...
    if not isdir(stat_result_dir):
        print(f'Aborting report generation. No stats directory detected at: {report_dir}')
        exit()
    nodes_stat_dirs = [join(stat_result_dir, f) for f in listdir(stat_result_dir) if isdir(join(stat_result_dir, f))]

    # Define task_id
    task_id_path = report_dir.split('/')
    task_id = task_id_path[-2].split('_')[-1] if task_id_path[-1] == '' else task_id_path[-1].split('_')[-1]
    ...
    # Time
    workload_start_time, workload_stop_time, test_start_time, test_stop_time = parse_time(stat_result_dir)

    # Hardware
    mems, cpu_info, commit_info = parse_hardware_info(nodes_stat_dirs)
    
    # Network
    network_infos, lnet_conf, lnet_info, multipath_conf, multipath_info = parse_network_info(nodes_stat_dirs)
    ...
    disks_mapping_info, nodes_mapping_info = parse_mapping_info(nodes_stat_dirs)
