...
function run_command()
{
	...
	"configure")
		...
		on_each_server sed -e "s/^.*MOTR_M0D_EXTRA_OPTS.*$/MOTR_M0D_EXTRA_OPTS=\'-g\ -I\'/" -i /etc/sysconfig/motr

		# disable addb2 network submit in kernel module
		on_each_node sed "s/.*MOTR_KMOD_EXTRA_PARAMS.*/MOTR_KMOD_EXTRA_PARAMS=\'addb2_net_disable=1\'/" -i /etc/sysconfig/motr

		# disable traced
		on_each_node sed 's/^MOTR_TRACED/#&/' -i /etc/sysconfig/motr

		# set default layout ID
		# XXX workaround, need to read from genders
	...
	"start_servers")
		ssh -n $CONFD_NODE systemctl start motr-server-confd
		on_each_server systemctl start motr
		;;
	"start_clients")
		on_each_client systemctl start motr-client
		;;
	"stop_clients")
		on_each_client systemctl stop motr-client
		;;
	"stop_servers")
		on_each_server systemctl stop motr
		;;
	"test")
		run_test
		;;
