				...
				bool               init_root)
{
	 ...
	 * by users (motr/setup, ut/ut_rpc_machine).
				   ...
				   struct m0_be_tx       *tx)
{
	...
	M0_ASSERT(pfid != NULL);
        ...
        /* We don't allow create in .motr and .motr/fid directory. */
				...
				struct m0_be_tx         *tx)
{
	...
	M0_ASSERT(cob != NULL);
        ...
        /* We don't allow link in .motr and .motr/fid directory. */
        if (m0_fid_cmp(pfid, &M0_MDSERVICE_SLASH_FID) < 0)
				  ...
				  struct m0_be_tx       *tx)
{
	...
	M0_LOG(M0_DEBUG, FID_F"/%.*s->"FID_F",%d cob",
        ...
        /* We don't allow unlink in .motr and .motr/fid directories. */
        if (m0_fid_cmp(pfid, &M0_MDSERVICE_SLASH_FID) < 0) {
                rc = -EOPNOTSUPP;
                goto out;
        }

        /* We don't allow to kill .motr dir. */
        if (m0_fid_eq(pfid, &M0_MDSERVICE_SLASH_FID) &&
            name->b_nob == strlen(M0_DOT_MOTR_NAME) &&
            !strncmp((char *)name->b_addr, M0_DOT_MOTR_NAME, (int)name->b_nob)) {
                rc = -EOPNOTSUPP;
                goto out;
        }
				  ...
				  struct m0_be_tx       *tx)
{
	...
	time(&now);
        ...
        /* We don't allow rename in/with .motr/fid directories. */
...
/**
   ...
   (cat /mnt/motr/.motr/1:5), parent fid is checked and
				  ...
				  struct m0_cob        **cob)
{
	...
	if (pfid == NULL)
        ...
        /*
          ...
          extracted from name.
         */
        if (m0_fid_eq(pfid, &M0_DOT_MOTR_FID_FID)) {
