 ...
 * The ref cnt held on src rec will be released in m0_fdmi__handle_release().
 *
 * But if error reply is detected, the ref cnt on sec rec will be released now.
 */
static int m0_fdmi__handle_reply(struct m0_fdmi_src_dock *sd_ctx,
				 struct m0_fdmi_src_rec  *src_rec,
				 int                      send_res)
{
...
static int m0_fdmi__handle_release(struct m0_uint128 *fdmi_rec_id)
{
	struct m0_fdmi_src_dock *src_dock;
	struct m0_fdmi_src_rec  *src_rec;
	uint64_t                 expected;
	...
	expected = src_dock->fsdc_instance_id;
	if (actual != expected) {
		M0_LOG(M0_WARN,
