...
static struct m0_container small_cont;
static struct m0_container large_cont;

int init_containers()
{
    struct m0_op *ops[2] = { NULL, NULL };
    int rc;
    ...
    m0_container_init(&small_cont, NULL, &M0_UBER_REALM, m0c);
    m0_container_init(&large_cont, NULL, &M0_UBER_REALM, m0c);

    m0_clovis_container_create(&small_cont, &ops[0]);
    m0_clovis_container_create(&large_cont, &ops[1]);

    m0_op_launch(ops, 2);
    return m0_op_wait(ops[0],
                           M0_BITS(M0_OS_FAILED, M0_OS_STABLE),
                           M0_TIME_NEVER);
}

void list_containers(struct m0_container *cont)
{
...
int main(int argc, char **argv)
{
    ...
    if (fork()) {
        ...
        struct m0_config cfg = {
            .mc_is_oostore = true,
            .mc_is_read_verify = false,
            .mc_ha_addr = ha_addr,    // IP@tcp:12345:45:1
            .mc_local_addr = local_addr2,    // IP@tcp:12345:44:101
            .mc_profile = prof,    // <0x7000000000000001:0>
            .mc_process_fid = "<0x7200000000000000:0>",
            .mc_tm_recv_queue_min_len = M0_NET_TM_RECV_QUEUE_DEF_LEN,
            .mc_max_rpc_msg_size = M0_RPC_DEF_MAX_RPC_MSG_SIZE,
            .mc_idx_service_id = M0_IDX_DIX,    // DIX
        ...
        default_layout = M0_DEFAULT_LAYOUT_ID;
