...
int save_kore_file(void)
{
	...
	ifd = open(MOTR_KO_CORE_IN_FILE_NAME, O_RDONLY);
	if (ifd == -1) {
		log_err("failed to open input file '%s': %s\n",
			MOTR_KO_CORE_IN_FILE_NAME, strerror(errno));
		return EX_NOINPUT;
	}
...
close:
	...
	if (rc == 0)
		log_debug("m0tr.ko core image saved into '%s'\n",
...
int main(int argc, char *argv[])
{
	...
	rc = m0_init(&instance);
	if (rc != 0) {
		log_err("failed to initialize libmotr\n");
		return EX_SOFTWARE;
	}
	...
	rc = M0_GETOPTS(progname, argc, argv,
	  ...
	  M0_STRINGARG('O',
		"output file name for m0tr.ko core image, if none is provided"
		", then " MOTR_KO_CORE_OUT_FILE_NAME " is used by default;"
	  ...
	  M0_VOIDARG('K',
		  "save m0tr.ko core image into a file, specified by -O option"
	  ...
	  M0_VOIDARG('S',
		  "discards action of '-K' option, i.e. don't save m0tr.ko"
