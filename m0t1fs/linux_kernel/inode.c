...
M0_INTERNAL bool m0t1fs_inode_is_dot_motr(const struct inode *inode)
{
	...
	return m0_fid_eq(m0t1fs_inode_fid(ci), &M0_DOT_MOTR_FID);
}
...
M0_INTERNAL bool m0t1fs_inode_is_dot_motr_fid(const struct inode *inode)
{
	...
	return m0_fid_eq(m0t1fs_inode_fid(ci), &M0_DOT_MOTR_FID_FID);
}
			     ...
			     struct m0_fop_cob *body)
{
	...
	if (S_ISREG(inode->i_mode)) {
		...
		inode->i_mapping->a_ops = &m0t1fs_aops;
	        ...
	        if ((m0t1fs_inode_is_dot_motr(inode) ||
		    m0t1fs_inode_is_dot_motr_fid(inode)) &&
		    csb->csb_oostore) {
	...
	if (!m0t1fs_inode_is_root(inode) && !m0t1fs_inode_is_dot_motr(inode) &&
	    !m0t1fs_inode_is_dot_motr_fid(inode)) {
