...
rcancel_motr_service_start()
{
	...
	echo "About to start Motr service, rcancel_md_redundancy=$rcancel_md_redundancy"
	...
	if [ $rc -ne 0 ]
	then
		echo "Failed to start Motr services: rc=$rc."
		return 1
	fi
	echo "Motr services have started successfully."
...
{
	local wt_write_file_base="$MOTR_M0T1FS_MOUNT_DIR/0:2222"
	local wt_write_new_file_base="$MOTR_M0T1FS_MOUNT_DIR/0:2299"
...
{
	local rt_read_file_base="$MOTR_M0T1FS_MOUNT_DIR/0:3333"
