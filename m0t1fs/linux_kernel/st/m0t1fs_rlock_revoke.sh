...
revoke_sandbox="$MOTR_M0T1FS_TEST_DIR/revoke_sandbox"
source_file="$revoke_sandbox/revoke_source"

revoke_motr_service_start()
{
...
revoke_during_io_test()
{
	...
	#cp $MOTR_TEST_LOGFILE "copyoflogfile.txt"
	#num=`grep -n "dd: " $MOTR_TEST_LOGFILE | grep "reading" | egrep "("Input\/output\ error")" | grep "$rt_file_base" | wc -l | cut -f1 -d' '`
...
main()
{
	...
	revoke_motr_service_start || return 1
