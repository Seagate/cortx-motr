...
fmio_pre()
{
	...
	file_to_create1="$MOTR_M0T1FS_MOUNT_DIR/0:11111"
...
fmio_io_test()
{
	...
	# This should be changed once MOTR-1166 lands into dev.
...
fmio_m0t1fs_mount()
{
	# XXX MOTR-704: Perform testing with non-oostore mode once MOTR-678 is
	# fixed
	if [ $1 -eq 0 ]
	then
...
failure_modes_test()
{
	...
	then
		...
		file_to_compare_m0t1fs="$MOTR_M0T1FS_MOUNT_DIR/0:10000"
		str="single file"
	...
	then
		...
		echo "--------------------------------------------------------"
		# @todo- Can not run repair in non-oostore mode till MOTR-678
                # gets fixed.
		if [ $OOSTORE -ne 1 ]
		then
			return 0
