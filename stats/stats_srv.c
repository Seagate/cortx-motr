...
/**
   ...
   - Updates of in-memory stats objects on requests from motr nodes
   ...
   - <b>Stats Service</b> Stats service which processes motr statistics
   ...
   - <b>Stats Query</b> Stats query request which returns latest updated
     stats objects value.
   - <b>Stats Clients</b> Motr administrative console/utilities.
   ...
   Motr system initialization and finalization (m0_init()/ m0_fini()).
   ...
   Motr nodes send stats (different 2-3 types) to stats service,
   verification of stats sent by Motr node with respective stats object.

   <hr>
   @section DLD-stats-svc-st System Test
   This tests whole monitoring infrastructure by running some sample IOs on
   on Motr and verify statistics collected by querying statistic service using
   ...
   - Since stats service is just a book keeping of stats information into
     ...
     values, it does not creates CPU usage overhead for motr nodes.
