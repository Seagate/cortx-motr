...
int m0_rpc_server_start(struct m0_rpc_server_ctx *sctx)
{
	...
	rc = m0_cs_init(&sctx->rsx_motr_ctx, sctx->rsx_xprts,
			sctx->rsx_xprts_nr, sctx->rsx_log_file, true);
	M0_LOG(M0_DEBUG, "cs_init: rc=%d", rc);
	if (rc != 0)
		goto fclose;

	rc = m0_cs_setup_env(&sctx->rsx_motr_ctx, sctx->rsx_argc,
			     sctx->rsx_argv);
	M0_LOG(M0_DEBUG, "cs_setup_env: rc=%d", rc);
	if (rc != 0)
		goto error;

	rc = m0_cs_start(&sctx->rsx_motr_ctx);
	if (rc == 0)
		return M0_RC(0);

error:
	m0_cs_fini(&sctx->rsx_motr_ctx);
fclose:
...
{
	return m0_motr_to_rmach(&sctx->rsx_motr_ctx);
}
#endif /* !__KERNEL__ */
