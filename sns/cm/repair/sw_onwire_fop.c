...
static int sns_repair_sw_fom_create(struct m0_fop *fop, struct m0_fom **m,
				    struct m0_reqh *reqh)
{
	struct m0_fop *r_fop;
	int            rc;

	r_fop = m0_fop_reply_alloc(fop, &sns_repair_sw_onwire_rep_fopt);
	if (r_fop == NULL)
...
const struct m0_fom_type_ops sns_repair_sw_fom_type_ops = {
	.fto_create = sns_repair_sw_fom_create
};

M0_INTERNAL void m0_sns_cm_repair_sw_onwire_fop_init(void)
{
        m0_cm_repreb_sw_onwire_fop_init(&sns_repair_sw_onwire_fopt,
					...
					&sns_repair_cmt);
        m0_cm_repreb_sw_onwire_fop_init(&sns_repair_sw_onwire_rep_fopt,
					&sns_repair_sw_fom_type_ops,
...
{
	m0_cm_repreb_sw_onwire_fop_fini(&sns_repair_sw_onwire_fopt);
	m0_cm_repreb_sw_onwire_fop_fini(&sns_repair_sw_onwire_rep_fopt);
}
...
{
	return m0_cm_repreb_sw_onwire_fop_setup(cm, &sns_repair_sw_onwire_fopt,
