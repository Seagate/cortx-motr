...
static void fd_permute(struct m0_fd_perm_cache *cache,
		       ...
		       uint64_t omega, uint32_t tree_int);
				       ...
				       struct m0_fd_perm_cache_grid **out)
{
	...
	M0_PRE(layout != NULL && out != NULL && layout->l_pver != NULL);
	...
	*out = cgrid;
	return M0_RC(0);
 err:
	m0_fd_cache_grid_destroy(layout, cgrid);
	return M0_ERR(rc);
}
		       ...
		       uint64_t omega, uint32_t tree_int)
{
	...
	if (!is_cache_valid(cache, omega, gfid)) {
		...
		/* Initialise PRNG. */
		rstate = m0_hash(seed->u_hi + gfid->f_key + tree_int) ^
