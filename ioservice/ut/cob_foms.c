...
#define SERVER_EP_ADDR              M0_UT_SERVER_EP_ADDR
#define CLIENT_EP_ADDR              M0_UT_CLIENT_EP_ADDR
...
static void cobfoms_preexisting_cob(void)
{
	...
	/*
	 * Get the value of cobfoms_ut::cu_gobindex to send cob_create
...
static void fom_create(struct m0_fom **fom, enum cob_fom_type fomtype)
{
	...
	base_fom = *fom;
	reqh = m0_cs_reqh_get(&cut->cu_sctx.rsx_motr_ctx);
