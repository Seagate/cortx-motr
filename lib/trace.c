...
m0_trace_subsys_list_to_mask(char *subsys_names, unsigned long *ret_mask)
{
	...
	while (p != NULL) {
		...
		if (m == 0) {
			m0_error_printf("motr: failed to initialize trace"
...
m0_trace_print_context_parse(const char *ctx_name)
{
	...
	m0_error_printf("motr: failed to initialize trace print context:"
			" invalid value '%s'\n", ctx_name);
...
#ifdef __KERNEL__
	pr_info("motr: trace print context: %s\n", ctx_name);
#endif
	return 0;
}
...
M0_INTERNAL int m0_trace_set_level(const char *level_str)
{
	...
	if (level == M0_NONE) {
		m0_error_printf("motr: incorrect trace level specification,"
...
m0_trace_record_print(const struct m0_trace_rec_header *trh, const void *buf)
{
	...
	if (rc != 0) {
		m0_error_printf("motr: failed to unpack trace descriptor args"
	...
	if (m0_trace_print_context == M0_TRACE_PCTX_SHORT)
		m0_error_printf("motr[%5.5u]: %5x %6s  [%s:%i:%s]  ",
				trh->trh_pid,
				(unsigned) (trh->trh_sp & 0xffff),
				m0_trace_level_name(td->td_level),
				...
				const void *tr_body, bool yaml_stream_mode)
{
	...
	if (rc > sizeof msg_buf)
		m0_error_printf("motr: %s: 'msg' is too big and has been"
	...
	if (rc != 0)
		m0_error_printf("motr: %s: failed to align msg: %s\n",
				__func__, msg_buf);
