...
/**
 * Extracts the file name, relative to a motr sources directory, from a
 * full-path file name. A motr source directory is detected by a name
 * "motr/".
 *
 ...
 *
 * @bug {
 *     This function doesn't search for the rightmost occurrence of "motr/"
 *     in a file path, if "motr/" encounters several times in the path the first
 ...
 *          file path, after motr source directory;
...
/**
 * Converts Motr function pointer in a form that can be stored somewhere (e.g.,
 ...
 * address at which Motr library is loaded.
 *
 * @pre "p" must be a pointer to Motr executable code or NULL.
 */
