 ...
 *
 * m0_threads_init() is usually called at the early stages of Motr
 ...
 *
 * Motr maintains a per-thread context needed for any Motr operation. A thread
 * with such a context is "Motr thread". A thread that calls m0_init() is a Motr
 * thread. A thread created by a call to m0_thread_init() (necessarily made by a
 * Motr thread) is a Motr thread.
 *
 * To turn any other thread into a Motr thread call m0_thread_adopt() before any
 * Motr call is made in the thread. m0_thread_shun() must be called before the
 * thread exits.
 *
