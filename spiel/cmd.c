				   ...
				   const struct m0_fid *pool_fid)
{
	M0_ENTRY();
	return M0_RC(spiel_pool_generic_handler(&spl->spl_core, pool_fid,
						CM_OP_DTREBALANCE, NULL,
						M0_REPREB_TYPE_SNS));
}
M0_EXPORTED(m0_spiel_sns_dtrebalance_start);
				      ...
				      const struct m0_fid *pool_fid)
{
	M0_ENTRY();
	return M0_RC(spiel_pool_generic_handler(&spl->spl_core, pool_fid,
						CM_OP_DTREBALANCE_RESUME, NULL,
						M0_REPREB_TYPE_SNS));
}
M0_EXPORTED(m0_spiel_sns_dtrebalance_continue);
				     ...
				     const struct m0_fid *pool_fid)
{
	M0_ENTRY();
	return M0_RC(spiel_pool_generic_handler(&spl->spl_core, pool_fid,
						CM_OP_DTREBALANCE_QUIESCE, NULL,
						M0_REPREB_TYPE_SNS));
}
M0_EXPORTED(m0_spiel_sns_dtrebalance_quiesce);
				  ...
				  struct m0_spiel_repreb_status **statuses)
{
	...
	M0_PRE(statuses != NULL);
	rc = spiel_pool_generic_handler(&spl->spl_core, pool_fid,
					CM_OP_DTREBALANCE_STATUS, statuses,
					M0_REPREB_TYPE_SNS);
	return rc >= 0 ? M0_RC(rc) : M0_ERR(rc);
}
M0_EXPORTED(m0_spiel_sns_dtrebalance_status);
				   ...
				   const struct m0_fid *pool_fid)
{
	M0_ENTRY();
	return M0_RC(spiel_pool_generic_handler(&spl->spl_core, pool_fid,
						CM_OP_DTREBALANCE_ABORT, NULL,
						M0_REPREB_TYPE_SNS));
}
M0_EXPORTED(m0_spiel_sns_dtrebalance_abort);
/***********************************************/
/*                 Byte count                  */
/***********************************************/
