 ...
 *
 *  - to inform Motr about hardware resources it should use, their roles and
 *    arrangement;
 *
 *  - to specify operational characteristics of Motr, such as fault-tolerance
 *    parameters;
 *
 *  - to issue commands to modify the cluster state: start and stop
 *    operation, format storage, etc.
 *
 * Motr stores information about cluster elements (hardware and software), their
 * arrangement, functions and operational characteristics in an internal
 * (replicated) data-base, called configuration data-base.
 *
 * Cluster state is changed by sending operation requests (fops) to Motr
 * services running on cluster nodes. This assumes that every node already runs
 * a mininal Motr process, which is started on node bootup and can be then
 * remotely commanded to start more services, as necessary.
 *
 * <hr>
 * @section spiel-dld-def Definitions
 *
 * - @b Configuration: Data-base describing Motr cluster in details required and
 ...
 *
 * Motr configuration data-base contains all the meta-data that is manipulated
 ...
 *
 * Spiel interface is exported from the standard Motr library, which uses Motr
 ...
 *
 * m0_cs_init(&motr, ...);
 * m0_cs_setup_env(&motr, ...);
 * m0_cs_start(&motr);
 *
 * m0_spiel_init(&spiel, m0_cs_reqh_get(&motr));
...
/**
 * Stops motr service. Stopping of Top Level RM is disallowed, the function
...
/**
 * Attaches device to the motr service and reports device object HA state found
...
/**
 * Re-configures process running on motr node
...
/**
 * Lists currently running services inside the motr process.
 ...
 *
 * When the library is loaded, motr_lib_init() function in it (if present) is
 ...
 * that the sum of IOS, MDS, and CAS instances present in configuration under
 * the profile is reported in m0_fs_stats::fs_svc_total, while the number of
 * polled and replied services is reported in m0_fs_stats::fs_svc_replied. So
...
struct m0_fs_stats {
	...
	m0_bcount_t fs_free_disk;   /**< free bytes on drives */
	m0_bcount_t fs_avail_disk;  /**< available bytes on drives */
	m0_bcount_t fs_total_disk;  /**< total bytes on drives */
	uint32_t    fs_svc_total;   /**< total number of IOS, MDS, and CAS */
	uint32_t    fs_svc_replied; /**< how many of them have replied */
};

/**
 * Fetches stats for filesystem pools within the profile. Spiel API
