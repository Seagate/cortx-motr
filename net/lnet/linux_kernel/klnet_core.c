...
/**
   ...
   The Lustre source package must be installed on the build
  ...
  @subsection KLNetCoreDLD-lspec-match-bits Match Bits for Buffer Identification
   ...
   by Motr file system clients, and the Core API has no business imposing
   ...
   permitted by the Motr style guide.)
   ...
   -# Generally speaking, API calls within the transport address space
      are protected by the serialization of the Motr Networking layer,
   ...
   -# The Motr Networking layer serialization does not always suffice, as
   ...
   -# The Motr LNet transport driver releases all kernel resources
