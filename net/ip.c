...
/** TODO: replace all sprintf with snprintf with rc*/
M0_INTERNAL int m0_net_ip_print(const struct m0_net_ip_addr *nia)
{
	...
	M0_LOG(M0_DEBUG, "frmt=%d num=[0x%"PRIx64",0x%"PRIx64"] port=%d",
		(int)na->nip_format, na->ip_n.ln[0],
		na->ip_n.ln[1], (int)na->nip_port);
	...
	if (na->nip_format == M0_NET_IP_LNET_FORMAT) {
		...
		if (rc >= M0_NET_IP_STRLEN_MAX)
			M0_LOG(M0_ERROR, "Name too long. Reqd: %d buf_len: %d", rc, M0_NET_IP_STRLEN_MAX);
	}
