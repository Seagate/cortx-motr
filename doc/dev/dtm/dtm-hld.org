...
*** clean startup
#+begin_src plantuml :file startup-clean.png :exports results
participant p1
participant p2
participant p3
participant hax

...p3 restarted by hare...
hax -> p3: start p3 process

hax -> p3: rpc session establish
p3 -> hax: rpc session establish

p3 -> hax: entry point request
hax -> p3: entry point reply [conf, profile]
p3 -> hax: **M0_CONF_HA_PROCESS_STARTING**
activate hax
...
deactivate p2
deactivate hax
...barrier **B1** is [ ∀ pi ∈ P | pi.state == RECOVERING ∧ \n∀ pi,pj ∈ P | pi ≠ pj ∧ pi.connected(pj) ]...
p1 -> p1: wait B1
p2 -> p2: wait B1
p3 -> p3: wait B1
...
p3 -> hax: **M0_CONF_HA_PROCESS_STARTED**
... no redo will happen today, start the process ...
p3 -> hax: **M0_CONF_HA_PROCESS_RECOVERED**
...
hax -> p1: ha state, p3 is in **ONLINE** state
hax -> p2: ha state, p3 is in **ONLINE** state
hax -> p3: ha state, p3 is in **ONLINE** state
... hax starts originators, startup is the same as p3 has except pi.state >= RECOVERING...
#+end_src

#+RESULTS:
[[file:startup-clean.png]]

**** Assumptions
- Written w.r.t. failures which are not reflected on the diagrams yet;
- Volatile participants synchronize it's start with connection
  requests issued by servers and reuses existing persistent
  participants logic;
- Volatile participants started after persistent participants start;
- Volatile participants stopped before persistent participants shutdown;
- [ha.EOS] during process operation, including start-up/shut-down
  procedures, process expects to that any event that hasn't been seen
  yet by the given process related to the process state in the cluster
  will be delivered to it in a "specific order" even in such process
  was offline/transient, wasn't able to receive messages etc.
  "Specific order" here *might* mean that events can be ordered by the
  arrival time seen on local hax side;
- After ~M0_CONF_HA_PROCESS_STARTING~ message is delivered to the
  local hax, it can start delivering ha (state change) messages to the
  participant in the cluster;
- Except given exceptions semantics of ~M0_CONF_HA_PROCESS_STARTING~
  and ~M0_CONF_HA_PROCESS_STARTED~ remains the same;
- Introduce ~M0_CONF_HA_PROCESS_RECOVERED~ message issued when the
  recovery is completed and indicating for hare side that ONLINE state
  can be broadcasted and then hare can start starting volatile
  participants;
- Shutdown procedure for persistent participants remains the same
  taking into account the whole cluster shutdown order defined above.

**** Highlights
The main goal of the startup/shutdown procedure definition is to
separate and synchronize service-, data flows- and system process
starts/stops related to DTM0 recovery and to address failure-related
scenarios in the future. Service flows are related to ha state changes
broadcasted to the whole cluster and establishment of rpc connections
needed for service and recovery traffic. Data flows cover REDO
messages logic and condition when the participant can start servicing
DIX GET requests.

As it can be seen from the diagram above service flow is bracketed by
the ~M0_CONF_HA_PROCESS_*~ messages inside intervals
~[M0_CONF_HA_PROCESS_STARTING, M0_CONF_HA_PROCESS_STARTED]~ and
~[M0_CONF_HA_PROCESS_RECOVERED..)~. Data traffic expected inside
~[M0_CONF_HA_PROCESS_STARTED, M0_CONF_HA_PROCESS_RECOVERED]~ interval.

The notion of barrier ~B1~ is introduced to synchronize all persistent
participants to the state when all of them have been established
connections to each other which is an indication that recovery or the
next processing phase can be started. Volatile participants here might
have a relaxed version of ~B1~ waiting for all participants to be in
~RECOVERING~ or next states.
