...
static int ad_dom_init(struct builder *b)
{
	...
	stob_location = m0_alloc(strnlen(b->b_stob_path,b->b_size) + 20);
...
static int builder_init(struct builder *b)
{
	...
	if (result != 0)
		return M0_ERR(result);
	ub->but_stob_domain_location = m0_alloc(strnlen(b->b_dom_path,b->b_size) + 20);
...
static int ctg_pver_fid_get(struct m0_fid *fid)
{
	struct m0_buf        key;
	struct m0_buf        val;
	uint8_t              kdata[M0_CAS_CTG_KEY_HDR_SIZE +
