...
static int find_next_avaliable_index(char *allocate)
{
	int j;
	for(j=0;j<=KV_NR;j++)
	{
...
//static int find_matching_index(struct m0_be_bnode *node, void *addr)
static int find_matching_index(struct m0_be_bnode *node, int check)
{
	int j;
	// struct be_btree_inlkey *temp = (struct be_btree_inlkey *) addr;
	// return (temp - node->bt_ik);
	for(j=0;j<=KV_NR;j++)
	{
		if( &node->bt_ik[j].inlkey == node->bt_kv_arr[check].btree_key )
		{
			break;
		}
	}
...
{
	bool retval = _0C(node->bt_header.hd_magic != 0) &&
	...
	if(retval != true)
	{
		M0_ASSERT(node->bt_header.hd_magic != 0);
		M0_ASSERT(m0_format_footer_verify(&node->bt_header, true) == 0);
		M0_ASSERT(btree_backlink_invariant(&node->bt_backlink, btree->bb_seg,
					     &btree->bb_cookie_gen));
		M0_ASSERT(node->bt_level <= BTREE_HEIGHT_MAX);
		M0_ASSERT(memcmp(&node->bt_backlink, &btree->bb_backlink,
			   sizeof node->bt_backlink) == 0);
		/* Expected occupancy. */
		M0_ASSERT(ergo(root, 0 <= node->bt_num_active_key &&
			 node->bt_num_active_key <= KV_NR));
		M0_ASSERT(ergo(!root, BTREE_FAN_OUT - 1 <= node->bt_num_active_key &&
			 node->bt_num_active_key <= KV_NR)) ;
		M0_ASSERT(m0_forall(i, node->bt_num_active_key,
			      node->bt_kv_arr[i].btree_key != NULL &&
			      node->bt_kv_arr[i].btree_val != NULL &&
			      m0_be_seg_contains(btree->bb_seg,
						 node->bt_kv_arr[i].
		...
		M0_ASSERT(ergo(!node->bt_isleaf,
			 m0_forall(i, node->bt_num_active_key + 1,
				   ...
				   m0_be_seg_contains(btree->bb_seg,
						      ...
						      bt_child_arr[i]))));
		/* Keys are in order. */
		M0_ASSERT(ergo(node->bt_num_active_key > 1,
			 m0_forall(i, node->bt_num_active_key - 1,
				   key_gt(btree, node->bt_kv_arr[i+1].btree_key,
					  node->bt_kv_arr[i].btree_key))));
	}
...
{	
	...
	if (node->bt_num_active_key > 0) {
		...
		if (btree->bb_ops->ko_type != M0_BBT_CAS_CTG) {
			...
			{
				for(i=0;i<count;i++)
				mem_update(btree, tx, &node->bt_ik[arr[i]],sizeof(*node->bt_ik));
			}
		}
	}
				     ...
				     bool                isleaf)
{
	...
	memset(p_node->allocated,0,KV_NR);
}
...
{
	int i;
	short int chidx_child[KV_NR];
	short int chidx_newchild[KV_NR];
	short int chidx_parent;
	short int count_child = 0;
	short int count_newchild = 0;
	...
	for (i = parent->bt_num_active_key + 1; i > index + 1; i--) {
		...
		// parent->bt_ik[i - 1] = parent->bt_ik[i - 2];
		// parent->allocated[i - 1] = parent->allocated[i - 2];
		// if(btree->bb_ops->ko_type != M0_BBT_CAS_CTG)
		// {
	...
	//*parent->bt_ik[index].inlkey = *((char *)child->bt_kv_arr[BTREE_FAN_OUT - 1].btree_key);
	
	if(btree->bb_ops->ko_type != M0_BBT_CAS_CTG && btree->bb_ops->ko_type != M0_BBT_CONFDB)
	{
		int tindex = find_next_avaliable_index(parent->allocated);
		int rindex = find_matching_index(child, BTREE_FAN_OUT - 1);
	...
	btree_node_update(parent, btree, tx, &chidx_parent, 1);
	btree_node_update(child, btree, tx, chidx_child, count_child);
	btree_node_update(new_child, btree, tx, chidx_newchild, count_newchild);
}
					 ...
					 struct be_btree_inlkey  *tinl)
{
	...
	while (i >= 0 &&
		...
		// if(btree->bb_ops->ko_type != M0_BBT_CAS_CTG)
		// {
		// 	node->bt_kv_arr[i+1].btree_key = &node->bt_ik[i+1].inlkey;
		// }
		i--;
	}
	...
	btree_node_update(node, btree, tx, &temp,1);
}
				   ...
				   struct be_btree_inlkey *tinl)
{
	...
	if (old_root->bt_num_active_key != KV_NR) {
		be_btree_insert_into_nonfull(btree, tx, old_root, kv, tinl);
	} else {
		...
		be_btree_insert_into_nonfull(btree, tx, new_root, kv, tinl);
					...
					short int	*count)
{
	...
	while (i < stop_index)
	{
		...
		// *dest->bt_ik[i + key_dest_offset].inlkey =
		// 			*((char *)src->bt_kv_arr[i + key_src_offset].btree_key);
		
		if(ktype != M0_BBT_CAS_CTG && ktype != M0_BBT_CONFDB)
		{
			int tindex = find_next_avaliable_index(dest->allocated);
			int rindex = find_matching_index(src, i + key_src_offset);
...
be_btree_merge_siblings(struct m0_be_tx    *tx,
			...
			unsigned int        idx)
{
	short int chidx_parent[KV_NR];
	short int chidx_node1[KV_NR];
	short int count_parent = 0;
	short int count_node1 = 0;
	...
	//*node1->bt_ik[tval].inlkey = *((char *)parent->bt_kv_arr[idx].btree_key);	
	...
	if (parent->bt_num_active_key == 0 && tree->bb_root == parent) {
		...
		/* Update affected memory regions */
		btree_node_update(parent, tree, tx, chidx_parent, count_parent);
	}

	btree_node_update(node1, tree, tx, chidx_node1, count_node1);
...
static void be_btree_move_parent_key_to_right_child(struct m0_be_bnode *parent,
						    ...
						    unsigned int        idx,
							uint64_t ktype,
							short int		*arr_parent,
							short int		*count_parent,
							short int		*arr_lch,
							short int		*count_lch,
							short int		*arr_rch,
							short int		*count_rch)
{
	...
	while (i > 0) {
		...
		// {
		// 	rch->bt_kv_arr[i].btree_key = &rch->bt_ik[i].inlkey;
		// }
	...
	//*rch->bt_ik[0].inlkey = *((char *)parent->bt_kv_arr[idx].btree_key);
	...
	// *parent->bt_ik[idx].inlkey =
	// 		*((char *)lch->bt_kv_arr[lch->bt_num_active_key-1].btree_key);

	if(ktype != M0_BBT_CAS_CTG && ktype != M0_BBT_CONFDB)
	{
		int tindex = find_next_avaliable_index(parent->allocated);
		int rindex = find_matching_index(lch,lch->bt_num_active_key-1);
		memcpy(parent->bt_ik[tindex].inlkey,lch->bt_kv_arr[lch->bt_num_active_key-1].btree_key,INLINE_KEY_SIZE);
		parent->allocated[tindex] = 1;
		arr_parent[*count_parent] = tindex;
		(*count_parent)++;
						   ...
						   short int		*count_rch)
{
	...
	lch->bt_kv_arr[lch->bt_num_active_key] =
					parent->bt_kv_arr[idx];
	// *lch->bt_ik[lch->bt_num_active_key].inlkey =
	// 				*((char *)parent->bt_kv_arr[idx].btree_key);
	...
	// *parent->bt_ik[idx].inlkey = *((char *)rch->bt_kv_arr[0].btree_key);
	// memcpy(parent->bt_ik[idx].inlkey,rch->bt_kv_arr[0].btree_key,64);
	// parent->allocated[idx] = 1;
	if(ktype != M0_BBT_CAS_CTG && ktype != M0_BBT_CONFDB)
	{
		int tindex = find_next_avaliable_index(parent->allocated);
		int rindex = find_matching_index(rch,0);
	...
	while (i < rch->bt_num_active_key - 1) {
		rch->bt_kv_arr[i] = rch->bt_kv_arr[i + 1];
		//*rch->bt_ik[i].inlkey = *((char *)rch->bt_kv_arr[i + 1].btree_key);
		
		// if(ktype != M0_BBT_CAS_CTG)
		// {
		// 	memcpy(rch->bt_ik[i].inlkey,rch->bt_kv_arr[i + 1].btree_key,64);
		// 	rch->allocated[i] = 1;
		// 	rch->bt_kv_arr[i].btree_key = &rch->bt_ik[i].inlkey;
		// }
				     ...
				     unsigned int          idx)
{
	...
	struct m0_be_bnode *rch;
	short int chidx_parent[KV_NR];
	short int chidx_lch[KV_NR];
	short int chidx_rch[KV_NR];
	short int count_parent=0;
	short int count_lch=0;
	short int count_rch=0;
	...
	if (pos == P_LEFT)
		be_btree_move_parent_key_to_left_child(parent, lch, rch, idx, tree->bb_ops->ko_type,
							chidx_parent, &count_parent, chidx_lch, &count_lch, chidx_rch, &count_rch);
	else
	...
	/* Update affected memory regions in tx: */
	btree_node_update(parent, tree, tx, chidx_parent, count_parent);
	btree_node_update(lch, tree, tx, chidx_lch, count_lch);
	btree_node_update(rch, tree, tx, chidx_rch, count_rch);
				   ...
				   struct btree_node_pos *bnode_pos)
{
	...
	if (bnode->bt_isleaf) {
		...
		bnode->allocated[find_matching_index(bnode,idx)] = 0;

		while (idx < bnode->bt_num_active_key - 1) {
			bnode->bt_kv_arr[idx] = bnode->bt_kv_arr[idx+1];
			//*bnode->bt_ik[idx].inlkey = *((char *)bnode->bt_kv_arr[idx+1].btree_key);
			
			// if(tree->bb_ops->ko_type != M0_BBT_CAS_CTG)
			// {
			// 	memcpy(bnode->bt_ik[idx].inlkey,bnode->bt_kv_arr[idx+1].btree_key,64);
			// 	bnode->allocated[idx] = 1;
			// 	bnode->bt_kv_arr[idx].btree_key = &bnode->bt_ik[idx].inlkey;
			// }
			++idx;
		}
		/*
		...
		if (bnode->bt_num_active_key == 0 && bnode != tree->bb_root)
			...
			/* Update affected memory regions in tx: */
			btree_node_update(bnode, tree, tx, NULL, -1);
	}
}
				    ...
				    bool		   left)
{
	...
	{	
		...
		memcpy(child->bnp_node->bt_kv_arr[child->bnp_index].btree_key,node->bt_kv_arr[index].btree_key,INLINE_KEY_SIZE);
		memcpy(node->bt_kv_arr[index].btree_key,temp.inlkey,INLINE_KEY_SIZE);

		//m0_format_footer_update(node);
		//m0_format_footer_update(child);
		mem_update(btree, tx, node->bt_kv_arr[index].btree_key,
			sizeof(*node->bt_ik));
		mem_update(btree, tx, child->bnp_node->bt_kv_arr[child->bnp_index].btree_key,
			sizeof(*child->bnp_node->bt_ik));
	...
	btree_node_keyval_update(child->bnp_node, btree, tx, child->bnp_index);
}
/**
			       ...
			       struct be_btree_key_val  *kv)
{
	if(btree->bb_ops->ko_type == M0_BBT_CAS_CTG || btree->bb_ops->ko_type == M0_BBT_CONFDB)
		...
		mem_free(btree, tx, kv->btree_val);
}
		       ...
		       uint64_t                   zonemask)
{
	...
	if ((cur_kv == NULL && optype != BTREE_SAVE_UPDATE) ||
		...
		if (op_tree(op)->t_rc == 0 &&
			...
			if(tree->bb_ops->ko_type != M0_BBT_CAS_CTG && tree->bb_ops->ko_type != M0_BBT_CONFDB)
			{
				memcpy(tinl.inlkey, key->b_addr, key->b_nob);
				new_kv.btree_key =&tinl.inlkey;
			}

			be_btree_insert_newkey(tree, tx, &new_kv, &tinl);
		}
	} else {
fi_exist:
