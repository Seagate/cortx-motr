...
static m0_bcount_t be_seg_get_dev_offset(struct m0_be_seg *seg)
{
	...
	if ((seg->bs_domain != NULL) && (((part_id == M0_BE_PTABLE_ENTRY_SEG0) &&
	     seg->bs_domain->bd_cfg.bc_part_cfg.bpc_stobs_cfg[M0_BE_DOM_PART_IDX_SEG0].bps_enble) ||
	    ((part_id == M0_BE_PTABLE_ENTRY_SEG1) &&
	     seg->bs_domain->bd_cfg.bc_part_cfg.bpc_stobs_cfg[M0_BE_DOM_PART_IDX_SEG1].bps_enble)))
