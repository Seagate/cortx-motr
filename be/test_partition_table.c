...
static struct m0_be_ptable_part_tbl_info pt={0};
...
static int init_parition_table()
{
	...
	/**  populate other tables like M0_BE_PTABLE_ENTRY_LOG,
	 * M0_BE_PTABLE_ENTRY_SEG0, M0_BE_PTABLE_ENTRY_SEG1, Balloc*/
	for (primary_part_index = 0;
		...
		else if (pt.pti_pri_part_info[primary_part_index].ppi_part_id ==
				    ...
				    m0_bcount_t partition_id)
{
	m0_bcount_t device_chunk_offset;
	M0_ENTRY();
	if (partition_id == M0_BE_PTABLE_ENTRY_LOG)
		device_chunk_offset = log_table[user_chunk_offset_index];
	else if (partition_id == M0_BE_PTABLE_ENTRY_SEG0)
		device_chunk_offset = seg0_table[user_chunk_offset_index];
	else if (partition_id == M0_BE_PTABLE_ENTRY_SEG1)
		device_chunk_offset = seg1_table[user_chunk_offset_index];
	else if (partition_id == M0_BE_PTABLE_ENTRY_BALLOC)
					     ...
					     m0_bcount_t partition_id)
{
	...
	if(partition_id ==   M0_BE_PTABLE_PARTITION_TABLE)
	...
	device_offset_in_bytes =
		calculate_device_offset( user_offset_in_bytes,
					 pt.pti_chunk_size_in_bits,
					 device_chunk_offset);
