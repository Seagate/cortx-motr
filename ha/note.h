 ...
 * events between HA and Motr core.
 *
 * Any HA-related event is represented as a state change of a configuration
 * object. Configuration objects are stored in the Motr configuration data-base
 * hosted by confd services and accessible to Motr and HA instances through
 ...
 *
 * On startup, Motr instance connects to confd and populates its local confc
 * with configuration objects. Then, Motr instance calls m0_ha_state_get() (one
 ...
 *
 * On detecting a failure, Motr core calls m0_ha_state_set(), which takes as an
 ...
 *
 * 2. HA notifies Motr about failure.
 *
 * When HA agrees about a failure, it sends to each Motr instance a
 * m0_ha_state_set_fopt fop. Motr replies with generic fop reply.
 *
...
struct m0_ha_msg_nvec {
	 ...
	 * |-------------------|-------------------------|-----------------|
	 *
	 * NOTE: Hare monitors a subset (`S`) of Motr conf objects.
	 *       Whenever HA state of _any_ of those objects changes,
	 *       Hare creates an nvec with current HA states of _all_
	 *       objects in `S` and broadcasts this nvec to Motr processes.
	 *       Upon receiving such nvec, Motr process compares it with
 ...
 * failures. Specifically, a Motr instance should not assume that the object
